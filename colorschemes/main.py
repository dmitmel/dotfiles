...
ThemeGenerator = Callable[[Theme, Path], None]

THEME_GENERATORS: list[ThemeGenerator] = []


def add_theme_generator(file_name: str | Path) -> Callable[[ThemeGenerator], None]:
  def decorator(generator: ThemeGenerator) -> None:
    THEME_GENERATORS.append(lambda theme, output_dir: generator(theme, output_dir / file_name))

  return decorator


@add_theme_generator("kitty.conf")
def generate_kitty(theme: Theme, output_path: Path) -> None:
...
def generate_termux(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
...
def generate_zsh(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:

    def write_color(key_name: str, color: Color) -> None:
      output.write(f"colorscheme_{key_name}={shlex.quote(color.hex)}\n")
    ...
    for color in theme.ansi_colors:
      output.write(f"  {shlex.quote(color.hex)}\n")
    output.write(")\n")


@add_theme_generator("vim.vim")
def generate_vim(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
...
def generate_setvtrgb(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
    for i in range(3):
...
def generate_xfce_terminal(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
    output.write("[Scheme]\n")
...
def generate_vscode(theme: Theme, output_path: Path) -> None:
  colors: dict[str, str] = {
...
@add_theme_generator("iterm.itermcolors")
def generate_iterm2(theme: Theme, output_path: Path) -> None:
...
def generate_css_variables(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
    output.write(":root {\n")
...
def generate_scss(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
...
def generate_prism_js(theme: Theme, output_path: Path) -> None:
  with open(__dir__ / "prismjs-theme-src.css") as src_file:
...
def generate_lua(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
...
@add_theme_generator("apple-terminal.terminal")
def generate_apple_terminal(theme: Theme, output_path: Path) -> None:
...
def generate_mintty(theme: Theme, output_path: Path) -> None:
  with open(output_path, "w") as output:
...
def main() -> None:
  theme: Theme = IniTheme(__dir__ / "data.ini")

  out_dir = __dir__ / "out"
  os.makedirs(out_dir, exist_ok=True)

  for generator in THEME_GENERATORS:
    generator(theme, out_dir)
