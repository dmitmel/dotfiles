...
lsp_ignition.setup_config('rust_analyzer', {
  ...
  on_new_config = function(config)
    local orig_handler = config.handlers['window/showMessage']
      or lsp.handlers['window/showMessage']
    ...
    config.handlers['window/showMessage'] = lsp_utils.wrap_handler_compat(
      function(err, result, ctx, opts)
        if
          ...
          and result.message:match('^overly long loop turn: ')
        then
          return
        end
  ...
  vim_user_commands = {
    ...
    LspRustCrateGraph = {
      bang = true,
      handler = function(call_info, client, bufnr)
        client.request(
    ...
    LspRustSyntaxTree = {
      ...
      handler = function(call_info, client, bufnr)
        ...
        local start_pos, end_pos =
          { call_info.line1, 1 }, { call_info.line2, #vim.fn.getline(call_info.line2) }
        if call_info.range ~= 0 then
          if vim.fn.visualmode() == 'v' then
            ...
            local start_mark, end_mark =
              { vim.fn.line("'<"), vim.fn.col("'<") }, { vim.fn.line("'>"), vim.fn.col("'>") }
