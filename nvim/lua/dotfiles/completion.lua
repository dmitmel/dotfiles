...
function _G._nvim_cmp_buffer_benchmark()
  local source = cmp.core.sources_by_name['buffer'][1]
  -- for _, buffer in ipairs(source.source:_get_buffers(source.source:_validate_options(source:get_config()))) do
  --   buffer:mark_all_lines_dirty()
  -- end
  local start_mem = vim.loop.resident_set_memory()
  local start_time = vim.loop.hrtime()
  source.source:complete({
    option = source:get_config().option or {},
    context = {
      cursor_before_line = '',
    ...
    print(
      string.format(
        '%d items%s, %f ms, %f MB',
        #result.items,
        #result.items ~= 17796 and ' (MISMATCH)' or '',
