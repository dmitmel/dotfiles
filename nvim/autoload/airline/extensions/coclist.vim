...
function! airline#extensions#coclist#init(ext) abort
  let g:coc_user_config = get(g:, 'coc_user_config', {})
  let g:coc_user_config['list.statusLineSegments'] = v:null

  call a:ext.add_statusline_func('airline#extensions#coclist#apply')
  call a:ext.add_inactive_statusline_func('airline#extensions#coclist#apply')

  for part in ['mode', 'args', 'name', 'cwd', 'loading', 'total']
    call airline#parts#define_function('coclist_' . part, 'airline#extensions#coclist#part_' . part)
  endfor
  call airline#parts#define_accent('coclist_mode', 'bold')
  ...
  let s:section_a = airline#section#create_left(['coclist_mode'])
  let s:section_b = airline#section#create(['coclist_name'])
  let s:section_c = airline#section#create(['%<', 'coclist_args', ' ', 'coclist_loading'])
  let s:section_x = airline#section#create(['coclist_cwd'])
  let s:section_y = airline#section#create(['#%L/', 'coclist_total'])
  let s:section_z = airline#section#create(['%p%%', 'linenr', 'maxlinenr'])
endfunction

function! airline#extensions#coclist#apply(builder, context) abort
...
let s:section_truncate_width = get(g:, 'airline#extensions#default#section_truncate_width', { 'b': 79, 'x': 60, 'y': 88, 'z': 45 })
...
function! airline#extensions#coclist#part_args() abort
  return get(b:list_status, 'args', '')
endfunction

function! airline#extensions#coclist#part_name() abort
  return get(b:list_status, 'name', '')
endfunction

function! airline#extensions#coclist#part_loading() abort
  return get(b:list_status, 'loading', '')
endfunction

function! airline#extensions#coclist#part_total() abort
  return get(b:list_status, 'total', '')
endfunction

function! airline#extensions#coclist#part_cwd() abort
