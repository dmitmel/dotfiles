...
syntax match kicadSymbol /[^[:space:]\b\n()]/

syntax region kicadList transparent matchgroup=kicadParen start="(" end=")"

" A token is basically a list with a name. The old regexp engine is MUCH faster
" for this pattern.
syntax match kicadToken /\%#=1\%(([[:space:]\b\n]*\)\@<=\w\+[[:space:]\b\n())]\@=/
...
syntax match kicadNumber /\%#=1[^[:space:]\b\n()]\@1<!-\?[[:digit:].]\+[^[:space:]\b\n())]\@!/
